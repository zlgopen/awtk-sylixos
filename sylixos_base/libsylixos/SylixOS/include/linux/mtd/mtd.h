...
static LW_INLINE int mtd_write_oob(struct mtd_info *mtd, loff_t to,
				struct mtd_oob_ops *ops)
{
...
static LW_INLINE void mtd_sync(struct mtd_info *mtd)
{
	if (mtd->_sync)
		mtd->_sync(mtd);
}
...
#ifndef SYLIXOS
static LW_INLINE uint32_t mtd_div_by_eb(uint64_t sz, struct mtd_info *mtd)
{
	do_div(sz, mtd->erasesize);
	return sz;
}

static LW_INLINE uint32_t mtd_mod_by_eb(uint64_t sz, struct mtd_info *mtd)
{
	return do_div(sz, mtd->erasesize);
}
#endif

static LW_INLINE int mtd_has_oob(const struct mtd_info *mtd)
{
	return mtd->_read_oob && mtd->_write_oob;
}

static LW_INLINE int mtd_can_have_bb(const struct mtd_info *mtd)
{
...
#else
static LW_INLINE void mtd_erase_callback(struct erase_info *instr)
{
	if (instr->callback)
...
static LW_INLINE int mtd_is_bitflip(int err) {
	return err == -EUCLEAN;
}

static LW_INLINE int mtd_is_eccerr(int err) {
	return err == -EBADMSG;
}

static LW_INLINE int mtd_is_bitflip_or_eccerr(int err) {
	return mtd_is_bitflip(err) || mtd_is_eccerr(err);
}

#endif /* __LINUX_MTD_MTD_H */
