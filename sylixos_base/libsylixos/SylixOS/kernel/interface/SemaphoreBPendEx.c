...
**--------------文件信息--------------------------------------------------------------------------------
**
** 文   件   名: SemaphoreBPendEx.c
**
** 创   建   人: Han.Hui (韩辉)
**
** 文件创建日期: 2007 年 07 月 21 日
**
** 描        述: 等待二进制型信号量 (高级接口，可以截获 PostEx 传送的简单消息)

** BUG
2007.07.21  加入 _DebugHandle() 功能。
2007.10.28  不能在调度器被锁定的情况下调用.
...
2009.03.10  因为修改了 errno 系统, 所以对一些 API 作出调整.
...
注意：当 ulTimeout == LW_OPTION_NOT_WAIT 时 API_SemaphoreBPend 还是不同于 API_SemaphoreBTryPend

      API_SemaphoreBTryPend 可以在中断中使用，而 API_SemaphoreBPend 不行
*********************************************************************************************************/
/*********************************************************************************************************
** 函数名称: API_SemaphoreBPendEx
** 功能描述: 等待二进制型信号量
** 输　入  : 
**           ulId            事件句柄
**           ulTimeout       等待时间
...
LW_API  
ULONG  API_SemaphoreBPendEx (LW_OBJECT_HANDLE  ulId, ULONG  ulTimeout, PVOID  *ppvMsgPtr)
{
