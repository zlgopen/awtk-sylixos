...
**--------------文件信息--------------------------------------------------------------------------------
**
** 文   件   名: SemaphoreBRelease.c
**
** 创   建   人: Han.Hui (韩辉)
**
** 文件创建日期: 2007 年 07 月 21 日
**
** 描        述: 释放二进制型信号量(高级函数，与 WIN32 API 非常相似)
...
/*********************************************************************************************************
** 函数名称: API_SemaphoreBRelease
** 功能描述: 释放二进制型信号量
** 输　入  : 
**           ulId                   事件句柄
**           ulReleaseCounter       释放的信号量个数
**           pbPreviousValue        保存原先的信号量状态，可以为 NULL
** 输　出  : 
** 全局变量: 
** 调用模块: 
                                           API 函数
...
LW_API  
ULONG  API_SemaphoreBRelease (LW_OBJECT_HANDLE  ulId, ULONG  ulReleaseCounter, BOOL  *pbPreviousValue)
{
    ...
    if (pbPreviousValue) {                                              /*  保存先前的数值              */
        *pbPreviousValue = (BOOL)pevent->EVENT_ulCounter;
    }
