...
static int fpu_emu(ARCH_REG_CTX *, ARCH_FPU_CTX *,
	mips_instruction);

static int fpux_emu(ARCH_REG_CTX *,
	ARCH_FPU_CTX *, mips_instruction, void __user **);
 ...
 * modules.
 */
int isBranchInstr(ARCH_REG_CTX *regs, struct mm_decoded_insn dec_insn,
		  unsigned long *contpc)
{
	...
	case cop1_op:
		...
		if (cpu_has_mips_r6 &&
			...
			bit = 0;
			fpr = &GET_CUR_THREAD_FPU_FPR[insn.i_format.rt];
	...
	case cop1x_op:
		if (insn.i_format.rs == bc_op) {
			...
			preempt_enable();
#else
            fcr31 = GET_CUR_THREAD_FPU_FCR31;
#endif
 ...
 */
static inline int cop1_64bit(ARCH_REG_CTX *xcp)
{
 ...
 */
static inline void cop1_cfc(ARCH_REG_CTX *xcp, ARCH_FPU_CTX *ctx,
			    mips_instruction ir)
{
	u32 fcr31 = ctx->fcr31;
	u32 value = 0;
 ...
 */
static inline void cop1_ctc(ARCH_REG_CTX *xcp, ARCH_FPU_CTX *ctx,
			    mips_instruction ir)
{
	u32 fcr31 = ctx->fcr31;
	u32 value;
...
static int cop1Emulate(ARCH_REG_CTX *xcp, ARCH_FPU_CTX *ctx,
...
emul:
	...
	case cop1_op:
		...
		case bc1nez_op:
			...
			cond = likely = 0;
			fpr = &GET_CUR_THREAD_FPU_FPR[MIPSInst_RT(ir)];
			bit0 = get_fpr32(fpr, 0) & 0x1;
			switch (MIPSInst_RS(ir)) {
			case bc1eqz_op:
...
static int fpux_emu(ARCH_REG_CTX *xcp, ARCH_FPU_CTX *ctx,
 ...
 */
static int fpu_emu(ARCH_REG_CTX *xcp, ARCH_FPU_CTX *ctx,
	mips_instruction ir)
{
 ...
 */
int fpu_emulator_cop1Handler(ARCH_REG_CTX *xcp, ARCH_FPU_CTX *ctx,
