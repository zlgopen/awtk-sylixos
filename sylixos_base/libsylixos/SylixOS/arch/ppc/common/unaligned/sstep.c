...
static nokprobe_inline long address_ok(ARCH_REG_CTX *regs,
...
static int do_fp_load(struct instruction_op *op, unsigned long ea,
		      ARCH_REG_CTX *regs, bool cross_endian)
{
	int err, rn, nb;
	union {
		int i;
...
#else
        LW_CPU_GET_CUR()->CPU_ptcbTCBCur->TCB_fpuctxContext.FPUCTX_fpuctxContext.FPUCTX_ulDreg[rn] = u.l[0];
#endif
	if (nb == 16) {
		/* lfdp */
		rn |= 1;
...
#else
		    LW_CPU_GET_CUR()->CPU_ptcbTCBCur->TCB_fpuctxContext.FPUCTX_fpuctxContext.FPUCTX_ulDreg[rn] = u.l[1];
#endif
	}
	preempt_enable();
	return 0;
...
#else
	    u.l[0] = LW_CPU_GET_CUR()->CPU_ptcbTCBCur->TCB_fpuctxContext.FPUCTX_fpuctxContext.FPUCTX_ulDreg[rn];
#endif
	if (nb == 4) {
		if (op->type & FPCONV)
...
#else
	        u.l[1] = LW_CPU_GET_CUR()->CPU_ptcbTCBCur->TCB_fpuctxContext.FPUCTX_fpuctxContext.FPUCTX_ulDreg[rn];
#endif
	}
	preempt_enable();
	if (unlikely(cross_endian)) {
...
#else
	    LW_CPU_GET_CUR()->CPU_ptcbTCBCur->TCB_dspctxContext.DSPCTX_dspctxContext.VECCTX_regs[rn] = u.v;
#endif
	preempt_enable();
	return 0;
}
...
#else
        u.v = LW_CPU_GET_CUR()->CPU_ptcbTCBCur->TCB_dspctxContext.DSPCTX_dspctxContext.VECCTX_regs[rn];
#endif
	preempt_enable();
	if (unlikely(cross_endian))
		do_byte_reverse(&u.b[ea & 0xf], size);
...
#ifdef __powerpc64__
static nokprobe_inline int emulate_lq(ARCH_REG_CTX *regs, unsigned long ea,
...
static nokprobe_inline int emulate_stq(ARCH_REG_CTX *regs, unsigned long ea,
				       int reg, bool cross_endian)
{
	int err;
...
static nokprobe_inline int do_vsx_load(struct instruction_op *op,
				       unsigned long ea, ARCH_REG_CTX *regs,
				       bool cross_endian)
{
	int reg = op->reg;
	u8 mem[16];
...
int emulate_dcbz(unsigned long ea, ARCH_REG_CTX *regs)
{
	int err;
...
static nokprobe_inline void set_cr0(const ARCH_REG_CTX *regs,
				    struct instruction_op *op)
{
	long val = op->val;
...
static nokprobe_inline void add_with_carry(const ARCH_REG_CTX *regs,
...
static nokprobe_inline void do_cmp_signed(const ARCH_REG_CTX *regs,
					  struct instruction_op *op,
...
static nokprobe_inline void do_cmp_unsigned(const ARCH_REG_CTX *regs,
					    struct instruction_op *op,
					    unsigned long v1,
...
static nokprobe_inline void do_cmpb(const ARCH_REG_CTX *regs,
				    struct instruction_op *op,
 ...
 */
static nokprobe_inline void do_popcnt(const ARCH_REG_CTX *regs,
				      struct instruction_op *op,
...
#ifdef CONFIG_PPC64
static nokprobe_inline void do_bpermd(const ARCH_REG_CTX *regs,
				      struct instruction_op *op,
				      unsigned long v1, unsigned long v2)
{
 ...
 * prtyw = 32, prtyd = 64
 */
static nokprobe_inline void do_prty(const ARCH_REG_CTX *regs,
				    struct instruction_op *op,
 ...
 * otherwise.
 */
int analyse_instr(struct instruction_op *op, const ARCH_REG_CTX *regs,
		  unsigned int instr)
{
 ...
 */
static nokprobe_inline int handle_stack_update(unsigned long ea, ARCH_REG_CTX *regs)
{
#ifndef SYLIXOS
#ifdef CONFIG_PPC32
	/*
 ...
 * fields in *regs.
 */
void emulate_update_regs(ARCH_REG_CTX *regs, struct instruction_op *op)
{
 ...
 */
int emulate_loadstore(ARCH_REG_CTX *regs, struct instruction_op *op)
{
	int err, size, type;
	int i, rd, nb;
	unsigned int cr;
...
#endif
		err = read_mem((unsigned long *)&regs->gpr[op->reg], ea, size, regs);
		if (!err) {
			if (op->type & SIGNEXT)
				do_signext((unsigned long *)&regs->gpr[op->reg], size);
			if ((op->type & BYTEREV) == (cross_endian ? 0 : BYTEREV))
				do_byterev((unsigned long *)&regs->gpr[op->reg], size);
		}
		break;
 ...
 */
int emulate_step(ARCH_REG_CTX *regs, unsigned int instr)
{
	struct instruction_op op;
	int r, err, type;
	unsigned long val;
