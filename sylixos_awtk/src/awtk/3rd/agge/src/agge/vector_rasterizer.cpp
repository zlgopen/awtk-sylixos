...
void sort_cells_x_ascending(IteratorT begin, IteratorT end) {
  // Yeah, bubblesort! 7th grade knowledge finally put to work! Seriously, if you append presorted
  // cells and then
...
void vector_rasterizer::line(int x1, int y1, int x2, int y2) {
  ...
  // Untested: we use top metric of cells required to draw the longest line given the current
  // bounds.
  ...
  if (ey2 == ey1) {
    ...
    if (x2 == x1) {
      ...
      if (lift) hline(current, tg_delta, ey1, x1, x_to, lift);
      ey1 += step;
      ...
      if (int dy_rest = fy2 - far) hline(current, tg_delta, ey1, x_to, x2, dy_rest);
    }
  }
...
void vector_rasterizer::append(const vector_rasterizer& source, int dx, int dy) {
  ...
  const cell& last =
      *(_cells.end() - 1);  // Relying on a guarantee that we had at least one cell prior this call.
...
void vector_rasterizer::sort(bool was_presorted) {
  ...
  if (was_presorted) {
    ...
    for (histogram::iterator i = _histogram_x.begin() + 1; i != _histogram_x.end(); ++i)
      *i += *(i - 1);
    phistogram_x--;
    for (cells_container::iterator i = _cells.begin(), j = _x_sorted_cells.begin(),
                                   e = _cells.end();
         i != e; ++i)
  ...
  for (histogram::iterator i = _histogram_y.begin() + 2; i != _histogram_y.end(); ++i)
    *i += *(i - 1);
  phistogram_y--;
  for (cells_container::iterator i = _x_sorted_cells.begin(), j = _cells.begin(),
                                 e = _x_sorted_cells.end();
       i != e; ++i)
...
AGGE_INLINE void vector_rasterizer::hline(cells_container::iterator& current,
                                          precise_delta& tg_delta, int ey, int x1, int x2, int dy) {
  int ex1 = x1 >> _1_shift;
  const int ex2 = x2 >> _1_shift;
