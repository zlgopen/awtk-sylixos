...
class precise_delta {
 public:
  AGGE_INLINE precise_delta(int numerator, int denominator) : _acc(0), _exp(0) {
    ...
    m--;  // Sacrifice precision to be agnostic to the rounding mode: we must not overflow on
          // increments!
    if (exp > 0x17)
      m <<= exp - 0x17;
    else if (exp >= 0x15)
