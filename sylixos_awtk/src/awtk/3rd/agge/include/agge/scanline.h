...
template <typename RendererT>
inline scanline_adapter<RendererT>::scanline_adapter(RendererT& renderer_,
                                                     raw_memory_object& covers, count_t max_length)
    : _renderer(renderer_),
      _x(0),
      _start_x(0),
      _start_cover(covers.get<cover_type>(max_length + 16) + 4) {
  _cover = _start_cover;
}
