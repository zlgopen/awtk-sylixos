...
class renderer {
 ...
 public:
  ...
  void operator()(BitmapT& bitmap_, const rect_i* window, const MaskT& mask,
...
template <typename BitmapT, typename BlenderT>
inline renderer::adapter<BitmapT, BlenderT>::adapter(BitmapT& bitmap_, const rect_i* window,
                                                     const BlenderT& blender)
    : _blender(blender),
      _offset_x(window ? window->x1 : 0),
      _limit_x((!window || static_cast<int>(bitmap_.width()) < width(*window) ? bitmap_.width()
                                                                              : width(*window)) +
               _offset_x),
      _bitmap(bitmap_),
      _offset_y(window ? window->y1 : 0),
      _limit_y((!window || static_cast<int>(bitmap_.height()) < height(*window) ? bitmap_.height()
                                                                                : height(*window)) +
               _offset_y) {
}
...
inline bool renderer::adapter<BitmapT, BlenderT>::set_y(int y) {
  if (y < _offset_y || _limit_y <= y) return false;
  _y = y;
...
template <typename BitmapT, typename BlenderT>
inline void renderer::adapter<BitmapT, BlenderT>::operator()(int x, int length,
  ...
  if (x + length > _limit_x) length = _limit_x - x;
  if (length > 0) _blender(_row + x, x, _y, length, covers);
}

template <unsigned _1_shift, typename ScanlineT, typename CellsIteratorT, typename AlphaFn>
AGGE_INLINE void sweep_scanline(ScanlineT& scanline, CellsIteratorT begin, CellsIteratorT end,
                                const AlphaFn& alpha) {
  int cover = 0;
  ...
  for (CellsIteratorT i = begin;;) {
    ...
    if (len && cover_m) scanline.add_span(x, len, alpha(cover_m));
  }
}

template <typename ScanlineT, typename MaskT, typename AlphaFn>
AGGE_INLINE void render(ScanlineT& scanline, const MaskT& mask, const AlphaFn& alpha, int offset,
                        int step) {
  for (int y = mask.min_y() + offset, limit_y = mask.min_y() + mask.height(); y < limit_y;
       y += step) {
...
inline void fill(BitmapT& bitmap_, const rect_i& area, const BlenderT& blender) {
  ...
  if (width > 0) {
    for (int y = agge_max(0, area.y1), limit_y = agge_min<int>(bitmap_.height(), area.y2);
         y < limit_y; ++y)
...
template <typename BitmapT, typename MaskT, typename BlenderT, typename AlphaFn>
void renderer::operator()(BitmapT& bitmap_, const rect_i* window, const MaskT& mask,
                          const BlenderT& blender, const AlphaFn& alpha) {
